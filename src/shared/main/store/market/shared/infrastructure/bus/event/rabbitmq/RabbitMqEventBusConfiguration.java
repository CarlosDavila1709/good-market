...
import store.market.shared.infrastructure.bus.event.DomainEventSubscribersInformation;
...
public class RabbitMqEventBusConfiguration {
    ...
    private Collection<Collection<Declarable>> declareQueuesAndBindings(
        ...
        return domainEventSubscribersInformation.all().stream().map(information -> {
            String queueName           = store.market.shared.infrastructure.bus.event.rabbitmq.RabbitMqQueueNameFormatter.format(information);
            String retryQueueName      = store.market.shared.infrastructure.bus.event.rabbitmq.RabbitMqQueueNameFormatter.formatRetry(information);
            String deadLetterQueueName = store.market.shared.infrastructure.bus.event.rabbitmq.RabbitMqQueueNameFormatter.formatDeadLetter(information);
